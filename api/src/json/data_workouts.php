...
$sql = "SELECT workouts.*,goals.goal_title AS goal_title, levels.level_title AS level_title FROM workouts,goals,levels WHERE workouts.workout_goal = goals.goal_id AND workouts.workout_level = levels.level_id AND workouts.workout_id='".$_GET["workout"]."' LIMIT 1";
mysqli_set_charset($connection, "utf8");
...
<?php
    ...
    if(empty($_GET["goal"]) && empty($_GET["level"]) && empty($_GET["workout"])){
